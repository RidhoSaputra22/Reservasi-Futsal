...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $req = mysqli_query($conn, "SELECT * FROM tb_lapangan WHERE id_lapangan = '$id'");
    $data = mysqli_fetch_assoc($req);

    $islapanganExist = mysqli_query($conn, "SELECT * FROM tb_lapangan WHERE id_lapangan = $id");

    if (mysqli_num_rows($islapanganExist) < 1) {
        echo '<script>window.location.replace("index.php?page=lapangan")</script>';
    }
} else {
    echo '<script>window.location.replace("index.php?page=lapangan")</script>';
}
...
<body>
    <section id="detail-product">
        <!-- <img src="../assets/img/lapangan/1716404639.jpg" alt=""> -->
        <div class="detail">
            ...
            <div class="image-wraper">
                ...
                $q = "SELECT * FROM tb_galeri WHERE id_lapangan = $id";
                ...
                ?>
                    ...
                    <div class="image p-2">
                        <div class="actions ">
                            <!-- EDIT BTN -->
                            <button type="button" id="edit-btn" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#edit-modal" data-foto="<?= $data['foto'] ?>" data-id="<?= $data['id_galeri'] ?>">
                            ...
                            <!-- DELETE BTN -->
                            <button type="button" id="delete-btn" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#delete-modal" data-id="<?= $data['id_galeri'] ?>">
